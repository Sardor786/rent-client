...
const NewClassified = () => {
		...
		(async () => {
			if (btn) {
				if (
					...
					type.value &&
					room.value &&
					square.value.length >= 2 &&
					addres.value.length >= 5 &&
					body.value.length >= 5
				) {
					const headers = {
						Token: window.localStorage.getItem("access_token"),
					...
					const classified = {
						...
						body: body.value,
						images: [
							one.value ? one.value : null,
							two.value ? two.value : null,
							three.value ? three.value : null,
							four.value ? four.value : null,
							five.value ? five.value : null,
							six.value ? six.value : null,
						],
					};
	...
	useEffect(() => {
		district.value === 0
			? setType({ data: [], value: 0 })
			: setType({ data: [1, 2], value: 0 });
	}, [district.value]);

	useEffect(() => {
		type.value === 0
			? setRoom({ value: 0 })
			: setRoom({ value: 0, data: [1, 2, 3, 4, 5, 6, "more..."] });
	}, [type.value]);
						...
						<div className="post--wrapper">
							...
							<div className="post_info">
								<div className="post_header">
									<input
										className="post_title post_input"
										type="text"
										onKeyUp={(e) =>
									...
									/>
									<input
										className="post_price post_input"
										type="number"
										onKeyUp={(e) =>
