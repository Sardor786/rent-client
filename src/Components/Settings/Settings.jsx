...
const Settings = () => {
	...
	useEffect(() => {
		(async () => {
			...
			const { data } = await axios.get(`${ADDRESS}/account/settings`, {
				headers,
			});
			if (data.data === null) {
	...
	function submit() {
		(async () => {
			if (newImg.name) {
				const { data } = await axios.post(
					`${ADDRESS}/account/settings`,
					{ button: true, id: user.user_id, img: newImg.name }
				);
				setUser(data.data);
			...
			if (usernameClass === "change_input success") {
				const { data } = await axios.post(
					`${ADDRESS}/account/settings`,
					{ button: true, username, id: user.user_id }
				);
				if (data.data) {
					setUser(data.data);
			...
			if (numberClass === "change_input success") {
				const { data } = await axios.post(
					`${ADDRESS}/account/settings`,
					{ button: true, number, id: user.user_id }
				);
				if (data.data) {
					setUser(data.data);
					window.localStorage.setItem(
			...
			if (
				passwordClass === "change_input success" &&
				newPasswordClass === "change_input success"
			) {
				const { data } = await axios.post(
					`${ADDRESS}/account/settings`,
					{ button: true, password, newPassword, id: user.user_id }
				);
				if (data.data) {
					setUser(data.data);
					window.localStorage.setItem(
	...
	function deleteAcc(a, b) {
		(async () => {
			if (a) {
				setVerify(true);
				await axios.post(`${ADDRESS}/account/settings`, {
					userDel: user,
				});
			}
			if (b) {
			...
			<div className="settings">
				<div className="container">
					<div className="card_setting">
						<Link className="exit-account" to="/account">
							<HiLogout />
						</Link>
						<div className="account_image">
							{newImg.name ? (
								<div className="have_img">
									<img
										src={`${ADDRESS}/images/` + newImg.name}
										alt="account_img"
										width="200"
										height="200"
									/>
				...
				<div className={verify ? "verify_del" : "none"}>
					<div className="del_content">
						...
						<input
							...
							onKeyUp={(e) => setVerifyCode(e.target.value)}
							type="number"
							placeholder="Code"
						/>
						<button
							onClick={() => deleteAcc(false, true)}
							className="delete_account"
						>
							Delete account
						</button>
